...
const styles = {
  ...
  subtitle: { fontSize: 14, color: "#4b5563", lineHeight: 1.65, maxWidth: 820 },
  ...
  // Slightly more “glass” + border, but same look & feel
  card: {
  ...
  smallText: { fontSize: 13, color: "#6b7280", lineHeight: 1.6 },
  ...
  // Make stat tiles a touch more premium (same colors, smoother)
  statBox: {
    borderRadius: 14,
    padding: 12,
    background:
      "linear-gradient(180deg, rgba(109,149,188,0.98), rgba(68,115,158,0.98))",
    border: "1px solid rgba(255,255,255,0.16)",
    boxShadow: "0 10px 18px rgba(11,30,50,0.14)",
  },
  statLabel: { fontSize: 12, color: "rgba(255,255,255,0.72)", fontWeight: 700 },
  statValue: { marginTop: 2, fontSize: 16, fontWeight: 800, color: "#ffffff" },
  ...
  moduleTitle: { fontSize: 15, fontWeight: 700, letterSpacing: -0.2 },
  moduleText: { fontSize: 13, color: "#4b5563", flexGrow: 1, lineHeight: 1.6 },
  moduleBtn: {
    marginTop: 4,
    padding: "7px 12px",
    borderRadius: 999,
    border: "1px solid rgba(11,30,50,0.55)",
    background: "#ffffff",
    color: "#0b1e32",
    fontSize: 13,
    cursor: "pointer",
  ...
  pillRow: { display: "flex", gap: 12, flexWrap: "wrap", marginTop: 10 },
  pill: (tone) => ({
    padding: "4px 10px",
    borderRadius: 999,
    fontSize: 12,
  ...
  sectionHint: { fontSize: 13, color: "#6b7280", lineHeight: 1.5 },
  ...
  inputFocusRing: {
    borderColor: "rgba(11,30,50,0.55)",
    boxShadow: "0 0 0 4px rgba(16,185,129,0.15)",
  },

  label: { fontSize: 12, color: "#374151", marginBottom: 0, fontWeight: 800 },
  ...
  btnPrimary: {
    padding: "10px 14px",
    borderRadius: 999,
    border: "1px solid rgba(11,30,50,0.75)",
    background: "linear-gradient(180deg,#0b1e32,#081528)",
    color: "#ffffff",
    fontSize: 13,
    cursor: "pointer",
    fontWeight: 900,
    boxShadow: "0 12px 22px rgba(11,30,50,0.18)",
    transition: "transform 0.12s ease, box-shadow 0.12s ease, opacity 0.12s ease",
  },
  btnGhost: {
    padding: "10px 14px",
    borderRadius: 999,
    border: "1px solid rgba(11,30,50,0.35)",
    background: "#ffffff",
    color: "#0b1e32",
    fontSize: 13,
    cursor: "pointer",
  ...
  calloutTitle: { fontSize: 13, fontWeight: 900, color: "#0b1e32" },
  calloutText: { marginTop: 6, fontSize: 13, color: "#0f172a", lineHeight: 1.6 },

  // Reusable surface (for little boxes)
  surface: {
    padding: 14,
    borderRadius: 16,
    border: "1px solid rgba(15,23,42,0.08)",
...
export default function DashboardPage() {
  ...
  // focus ring (inline-style friendly)
  ...
  const runStravaSync = async () => {
    ...
    try {
      ...
      setSyncState({
        status: "ok",
        msg: language === "de" ? "Sync abgeschlossen ✅" : "Sync complete ✅",
      });
      await loadRunStats();
    } catch (e) {
  ...
  const dynamicStatsRowStyle = {
    ...styles.statsRow,
    gridTemplateColumns: isNarrow ? "1fr" : styles.statsRow.gridTemplateColumns,
  };

  const dynamicModulesGridStyle = {
    ...styles.modulesGrid,
    gridTemplateColumns: isNarrow ? "1fr" : styles.modulesGrid.gridTemplateColumns,
  };

  return (
    <div style={styles.page}>
      {/* minimal CSS for hover/active polish (keeps your inline-style approach) */}
      <style>{`
        .lx-btn:hover { filter: brightness(1.01); }
        .lx-btn:active { transform: translateY(1px); }
        .lx-module:hover { transform: translateY(-2px); box-shadow: 0 10px 24px rgba(15,23,42,0.08); }
      `}</style>
      ...
      <div style={dynamicLayoutStyle}>
        <section>
          <div style={styles.card}>
            ...
            <div style={dynamicStatsRowStyle}>
          ...
          {/* --- prettier manual entry --- */}
          <div style={{ ...styles.card, marginTop: 18 }}>
            <div style={styles.sectionHeaderRow}>
              <div>
                <div style={styles.sectionTitle}>{t.manualEntryTitle}</div>
                <div style={styles.sectionHint}>{t.logHint}</div>
              </div>

              <div style={{ fontSize: 12, color: "#6b7280" }}>
                {runDate
                  ? new Date(runDate).toLocaleDateString(
            ...
            <div
              style={{
                ...styles.formGrid,
                gridTemplateColumns: isNarrow ? "1fr" : styles.formGrid.gridTemplateColumns,
              }}
            >
              <div style={styles.field}>
                ...
                <input
                  style={{
                    ...styles.input,
                    ...(focusedField === "date" ? styles.inputFocusRing : null),
                  }}
                  type="date"
                  value={runDate}
              ...
              <div style={styles.field}>
                ...
                <input
                  style={{
                    ...styles.input,
                    ...(focusedField === "km" ? styles.inputFocusRing : null),
                  }}
                  inputMode="decimal"
                  placeholder={language === "de" ? "z.B. 5,2" : "e.g. 5.2"}
                  value={distanceKm}
              ...
              <div style={styles.field}>
                ...
                <input
                  style={{
                    ...styles.input,
                    ...(focusedField === "dur" ? styles.inputFocusRing : null),
                  }}
                  inputMode="decimal"
                  placeholder={language === "de" ? "z.B. 32" : "e.g. 32"}
                  value={durationMin}
            ...
            <div style={styles.helperRow}>
              <span>{language === "de" ? "Tipp: Komma oder Punkt ist okay." : "Tip: comma or dot both work."}</span>
              <span>{language === "de" ? "Speichern zählt sofort in die Woche." : "Saving counts immediately."}</span>
            </div>
              ...
              {saveState.msg ? (
                <span
                  style={styles.statusBadge(
                    saveState.status === "error" ? "error" : "ok"
                  )}
                >
                  {saveState.msg}
                </span>
          ...
          <div style={{ ...styles.card, marginTop: 18 }}>
            ...
            <div style={dynamicModulesGridStyle}>
              <div className="lx-module" style={styles.moduleCard}>
        ...
        <aside style={styles.card}>
          <div style={{ ...styles.surface, marginTop: 0 }}>
